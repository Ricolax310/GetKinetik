...
- **Hotspots observed (with coordinates):** 54,982
- **Hotspots without asserted location:** 1,903
- **Hotspots flagged (any heuristic):** 11,291 (20.54%)

## Executive summary

1. **527 single-coordinate stacks of ≥10 hotspots** on 54,982 located units — the largest stack holds **540 hotspots on one coordinate** (§1 lists keys your registry team can grep today).
...
|---|---:|---|
| Hotspots with asserted coordinates | 54,982 | unchanged vs last run |
| Single-coordinate stacks (≥10 hotspots) | 527 | unchanged vs last run |
| Largest single-coordinate stack | 540 | unchanged vs last run |
| Fleet share flagged (any heuristic) | 20.54% | unchanged vs last run |
...
1. **527 coordinates host ≥10 hotspots each.** H3 snapping makes small shared-coordinate groups normal; stacks this size are the documented hotspot-stacking pattern and justify a registry cross-check.
2. **1,903 hotspots have no asserted location** — they exist on-chain but not on the map.

---

## 1. Largest single-coordinate stacks — 527 total
...
_…and 517 more in the snapshot file._

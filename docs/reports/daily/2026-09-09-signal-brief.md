...
> 2026-09-09 · derived from public audit snapshots only. Signals → report (not narrative-first).
...
• Geodnet flagged fleet share declined from 28% to 27%.
...
- **Geodnet** — exact (lat,lng) duplicate groups: 1,010 (-136 (-11.9%))
- **Geodnet** — entities on public map: 19,488 (-141 (-0.7%))
- **Geodnet** — fleet share flagged (any heuristic): 27.40% (-0.34 pp (-1.2%))
...
- **Hivemapper** — 52.15% of UI-reported HONEY sits in the top 20 visible SPL accounts (Solana RPC cap) — economic *shape* for treasury/MM review, not a contributor GPS read.
...
Public read on Geodnet: 1146 exact (lat,lng) duplicate groups on 19,629 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

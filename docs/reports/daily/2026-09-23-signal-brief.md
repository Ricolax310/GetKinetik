...
> 2026-09-23 · derived from public audit snapshots only. Signals → report (not narrative-first).
...
- **Geodnet** — exact (lat,lng) duplicate groups: 1,000 (-79 (-7.3%))
- **Geodnet** — entities on public map: 19,509 (-85 (-0.4%))
- **Geodnet** — fleet share flagged (any heuristic): 27.35% (-0.19 pp (-0.7%))
- **WeatherXM** — entities on public map: 8,582 (+6 (+0.1%))
...
Public read on Geodnet: 1079 exact (lat,lng) duplicate groups on 19,594 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

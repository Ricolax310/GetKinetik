...
> 2026-07-25 · derived from public audit snapshots only. Signals → report (not narrative-first).
...
- **Geodnet** — entities on public map: 19,598 (+9 (+0.0%))
- **WeatherXM** — cells over designed capacity: 288 (-1 (-0.3%))
- **DIMO** — entities on public map: 158,374 (+13 (+0.0%))
...
Public read on Geodnet: 9 exact (lat,lng) duplicate groups on 19,589 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

...
> 2026-07-02 · derived from public audit snapshots only. Signals → report (not narrative-first).

## Today's Read

• Geodnet duplicate-coordinate groups rose from 7 to 10.
• WeatherXM over-capacity cells declined from 289 to 288.
• Helium IoT single-coordinate hotspot stacks declined from 4,861 to 4,860.
...
- **Geodnet** — exact (lat,lng) duplicate groups: 10 (+3 (+42.9%))
...
Public read on Geodnet: 7 exact (lat,lng) duplicate groups on 19,577 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

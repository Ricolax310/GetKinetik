...
> 2026-08-05 · derived from public audit snapshots only. Signals → report (not narrative-first).
...
- **DIMO** — entities on public map: 158,418 (+7 (+0.0%))
- **Helium IoT** — single-coordinate stacks (≥10 hotspots): 4,860 (+1 (+0.0%))
- **Helium Mobile** — entities on public map: 54,903 (+20 (+0.0%))
- **Helium Mobile** — largest single-coordinate stack: 505 (+8 (+1.6%))
- **Hivemapper** — 48.22% of UI-reported HONEY sits in the top 20 visible SPL accounts (Solana RPC cap) — economic *shape* for treasury/MM review, not a contributor GPS read.
...
Public read on Geodnet: 6 exact (lat,lng) duplicate groups on 19,573 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

...
> 2026-06-02 · derived from public audit snapshots only. Signals → report (not narrative-first).

## Today's Read

• Natix Network zero-detection days rose from 32 to 33.
• Natix Network visible HONEY concentration remains near 72% of visible supply.
• Natix Network registered drivers rose from 271,400 to 271,401.
...
- **Geodnet** — 10 exact (lat,lng) duplicate groups on 19,612 public stations — each row in §1 is one coordinate pair your registry team can grep today.
...
Public read on WeatherXM: 288 cells exceed designed capacity — §1 lists H3 indices + map centers for your ops queue. — does that match your internal view, or is the public feed expected to behave this way?
...
- **Natix Network:** `docs/reports/natix-sybil-report.md` · Natix Coverage Map metrics feed (coverage.natix.network/coverage/v1/metrics) + Solana NATIX SPL mint

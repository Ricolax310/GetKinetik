...
- **Geodnet** — entities on public map: 19,582 (+8 (+0.0%))
- **WeatherXM** — cells over designed capacity: 290 (+3 (+1.0%))
- **DIMO** — entities on public map: 158,282 (+9 (+0.0%))
- **Hivemapper** — 50.65% of UI-reported HONEY sits in the top 20 visible SPL accounts (Solana RPC cap) — economic *shape* for treasury/MM review, not a contributor GPS read.
...
Public read on Geodnet: 7 exact (lat,lng) duplicate groups on 19,574 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

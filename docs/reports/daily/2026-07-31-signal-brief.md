...
> 2026-07-31 · derived from public audit snapshots only. Signals → report (not narrative-first).
...
- **DIMO** — entities on public map: 158,407 (+7 (+0.0%))
- **Helium IoT** — single-coordinate stacks (≥10 hotspots): 4,858 (-1 (-0.0%))
- **Helium Mobile** — largest single-coordinate stack: 497 (-1 (-0.2%))
- **Hivemapper** — 48.95% of UI-reported HONEY sits in the top 20 visible SPL accounts (Solana RPC cap) — economic *shape* for treasury/MM review, not a contributor GPS read.

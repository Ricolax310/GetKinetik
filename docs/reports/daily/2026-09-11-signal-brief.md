...
• Geodnet: 1011 exact (lat,lng) duplicate groups on 19,486 public stations — each row in §1 is one coordinate pair your registry team can grep today.
...
- **Helium Mobile** — entities on public map: 55,642 (+9 (+0.0%))
- **Geodnet** — 1011 exact (lat,lng) duplicate groups on 19,486 public stations — each row in §1 is one coordinate pair your registry team can grep today.
- **Hivemapper** — 52.56% of UI-reported HONEY sits in the top 20 visible SPL accounts (Solana RPC cap) — economic *shape* for treasury/MM review, not a contributor GPS read.

...
> 2026-09-25 · derived from public audit snapshots only. Signals → report (not narrative-first).
...
- **Geodnet** — exact (lat,lng) duplicate groups: 1,003 (-2 (-0.2%))
- **Geodnet** — entities on public map: 19,495 (-14 (-0.1%))
- **WeatherXM** — 300 cells exceed designed capacity — §1 lists H3 indices + map centers for your ops queue.
...
Public read on Geodnet: 1005 exact (lat,lng) duplicate groups on 19,509 public stations — each row in §1 is one coordinate pair your registry team can grep today. — does that match your internal view, or is the public feed expected to behave this way?

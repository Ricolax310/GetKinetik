...
> Last updated: **2026-05-13** — **`@getkinetik/verify@0.1.0` is LIVE on npm.** Public install path now exists: `npm i @getkinetik/verify`. Page at https://www.npmjs.com/package/@getkinetik/verify renders the full README; tarball is 11.9 kB (7 files, 35.4 kB unpacked). Scope went `@kinetik` (squatted by a dormant 0-package user) → `@getkinetik` (claimed cleanly, matches the .app domain, reads as the natural namespace for every future package). Publish was 2FA-gated via WebAuthn / Windows Hello security key; smoketest (27/27) was enforced as `prepublishOnly`. Cryptographic-contract version-bump policy codified in [`docs/PUBLISHING_VERIFY.md`](./docs/PUBLISHING_VERIFY.md). **What this changes:** until today, the only verification path for partners was the `verify-device` webhook (trusts our infra) or copying `landing/verify/verifier.js` into their codebase (drift risk). As of now, any partner can `npm install` a byte-equivalent verifier with zero dependency on getkinetik.app. The credibility unlock that converts "here's our API" into "here's our npm package." Bureau positioning is materially stronger today than it was yesterday.
>

...
> Last updated: **2026-04-28 (afternoon)** — **v1.3.2 built, NOT released — phone test surfaced two bugs.** EAS build `3dd83d04` finished, APK on disk at `artifacts/getkinetik-v1.3.2.apk` (SHA256 `66fbaa5370589900f4675155af05b6bbe72e76c60b24da10dd0c87d3da35c588`). Live phone test on real device confirmed (a) v1.3.2 IS installed, (b) DIMO redirect URI architectural fix works (consent screen now reachable, was dead-ended on v1.3.0/v1.3.1), but ALSO surfaced two bugs that block public release: **(1)** Gemstone still jitters at rest — root cause traced to a unit mismatch (Reanimated `useAnimatedSensor` returns m/s² on Android, but the v1.3.1 swap kept g-unit constants below the sensor read). Fix: convert m/s² → g via `/9.80665` before clamp/deadzone. Committed in `f1af7cd`, **NOT built yet**. **(2)** DIMO OAuth handoff back into the app drops the wallet param — Custom Tab interception is failing, the bounce page is rendering as the de-facto happy path, and the manual "Return to App" button hard-coded its href without `window.location.search`, so the deep-link the app received had no `wallet=0x…` and DIMO stayed "not active" after return. Bounce-page fix committed and live on Cloudflare in `7adf2af`. **No app rebuild needed for the bounce-page fix** — same v1.3.2 APK on the user's phone should now complete the DIMO round-trip when retested. **Pending: v1.3.3 EAS build** (will combine the gem unit-conversion fix with whatever further DIMO openAuthSessionAsync investigation surfaces). **All decisions on hold pending user phone retest.**

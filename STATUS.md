...
│
└── verify/              # @getkinetik/verify — publishable npm package (added 2026-05-04)
    ├── package.json     # name @getkinetik/verify, MIT, ESM, type roots — currently private: true
...
- **Push `@getkinetik/verify` to npm?** Package is built, smoketested, and `private: true`. Pushing makes the cryptographic verification path available to any partner without copying source files. Recommended once one partner has agreed to integrate (so the v0.1.0 release has a real consumer).
